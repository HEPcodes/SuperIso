...
{
	char name[50];
	double m0,m32,tanb,A0,sgnmu,mtop,mbot,alphas_mz,delta0,omega;
	...
	if(!test_file(ISAJET)) 
	{
		printf("\"%s\" is absent. Please check the ISAJET path or comment \"#define USE_ISAJET\" in amsb.c\n",ISAJET);
		filesOK=0;
	}
#endif
#ifdef USE_SOFTSUSY
	...
	{
		printf("\"%s\" is absent. Please check the SOFTSUSY path or comment \"#define USE_SOFTSUSY\" in amsb.c\n",SOFTSUSY);
		filesOK=0;
	}
#endif
#ifdef USE_SUSPECT
	...
	{
		printf("\"%s\" is absent. Please check the SUSPECT path or comment \"#define USE_SUSPECT\" in amsb.c\n",SUSPECT);
		filesOK=0;
	}
#endif
#ifdef USE_SPHENO
	if(!test_file(SPHENO)) 
	{
		printf("\"%s\" is absent. Please check the SPHENO path or comment \"#define USE_SPHENO\" in amsb.c\n",SPHENO);
		filesOK=0;
	}
#endif
	...
	{
		printf("\"%s\" is absent. Please check the HBwithFH path or comment \"#define USE_HIGGSBOUNDS\" in amsb.c\n",HBwithFH);
		filesOK=0;
	}
#endif
	if(!filesOK) return 1;

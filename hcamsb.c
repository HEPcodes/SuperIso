...
int main(int argc,char** argv)
{
	...
	{
		printf("\"%s\" absent. Please check the HIGGSBOUNDS path or comment \"#define USE_HIGGSBOUNDS\" in hcamsb.c\n",HIGGSBOUNDS);
		filesOK=0;
	};
	if(!test_file(HDECAY)) 
	{
		printf("\"%s\" absent. Please check the HDECAY path or comment \"#define USE_HIGGSBOUNDS\" in hcamsb.c\n",HDECAY);
		filesOK=0;
	};
#endif
	if(!filesOK) return 1;

...
int main(int argc,char** argv)
{
	...
	if(!test_file(ISAJET)) 
	{
		printf("\"%s\" is absent. Please check the ISAJET path or comment \"#define USE_ISAJET\" in hcamsb.c\n",ISAJET);
		filesOK=0;
	}
#endif
	...
	{
		printf("\"%s\" is absent. Please check the HBwithFH path or comment \"#define USE_HIGGSBOUNDS\" in hcamsb.c\n",HBwithFH);
		filesOK=0;
	}
#endif
	if(!filesOK) return 1;

...
/*--------------------------------------------------------------------*/
/* Calculation of the observables, corresponding to a mSUGRA point generated by SOFTSUSY and/or ISAJET and/or SUSPECT and/or SPHENO */
/*--------------------------------------------------------------------*/
...
int main(int argc,char** argv)
{
	...
	double m0,m12,tanb,A0,sgnmu,mtop,mbot,alphas_mz,delta0,omega;
  	...
  	{  
		sscanf(argv[1],"%lf",&m0);
     		...
     		if(argc>6) sscanf(argv[6],"%lf",&mtop); else mtop=173.3;   
	...
	if(!test_file(ISAJET)) 
	{
		printf("\"%s\" absent. Please check the ISAJET path or comment \"#define USE_ISAJET\" in msugra.c\n",ISAJET);
		filesOK=0;
	};
#endif
#ifdef USE_SOFTSUSY
	...
	{
		printf("\"%s\" absent. Please check the SOFTSUSY path or comment \"#define USE_SOFTSUSY\" in msugra.c\n",SOFTSUSY);
		filesOK=0;
	};
#endif
#ifdef USE_SUSPECT
	...
	{
		printf("\"%s\" absent. Please check the SUSPECT path or comment \"#define USE_SUSPECT\" in msugra.c\n",SUSPECT);
		filesOK=0;
	};
#endif
#ifdef USE_SPHENO
	if(!test_file(SPHENO)) 
	{
		printf("\"%s\" absent. Please check the SPHENO path or comment \"#define USE_SPHENO\" in msugra.c\n",SPHENO);
		filesOK=0;
	};
#endif
	...
	{
		printf("\"%s\" absent. Please check the HIGGSBOUNDS path or comment \"#define USE_HIGGSBOUNDS\" in msugra.c\n",HIGGSBOUNDS);
		filesOK=0;
	};
	if(!test_file(HDECAY)) 
	{
		printf("\"%s\" absent. Please check the HDECAY path or comment \"#define USE_HIGGSBOUNDS\" in msugra.c\n",HDECAY);
		filesOK=0;
	};
#endif
	if(!filesOK) return 1;
...
#ifdef USE_SOFTSUSY
	sprintf(name,"msugra_softsusy%d.tmplha",getpid());
	...
	sprintf(name,"rm msugra_softsusy%d.tmplha",getpid());
	system(name);		
#endif

#ifdef USE_ISAJET
	sprintf(name,"msugra_isajet%d.tmplha",getpid());
	...
	sprintf(name,"rm msugra_isajet%d.tmplha",getpid());
	system(name);
#endif	

#ifdef USE_SUSPECT
	sprintf(name,"msugra_suspect%d.tmplha",getpid());
	suspect_sugra(m0, m12, tanb, A0, sgnmu, mtop, mbot, alphas_mz, name);
	...
	sprintf(name,"rm msugra_suspect%d.tmplha",getpid());
	system(name);		
#endif

#ifdef USE_SPHENO
	sprintf(name,"msugra_spheno%d.tmplha",getpid());
	spheno_sugra(m0, m12, tanb, A0, sgnmu, mtop, mbot, alphas_mz, name);
	...
	sprintf(name,"rm msugra_spheno%d.tmplha",getpid());
	system(name);		
#endif

...
/* C0_spec and C1_spec: respectively LO and NLO contributions of the Wilson coefficients at scale mu_spec=O(sqrt(lambda_h*mb)) */
{
	...
	for (ie=1;ie<=8;ie++) C[ie]=C0[ie] + alpha_s_mub/4./pi*C1[ie];
	...
	float H8a7= 4./9.*pi*pi*f_B*f_K_orth/F_Kstar/m_B/lambda_B*um1_orth;
	
	complex float H1a7= 4./9.*pi*pi*f_B*f_K_orth/F_Kstar/m_B/lambda_B*(ubarm1_orth-h_spec);
	...
	complex float a7c=C[7] + alpha_s_mub/4./pi*4./3.*(C0[1]*G1a7+C0[8]*G8a7) + alpha_s_muspec/4./pi*4./3.*(C0_spec[8]*H8a7+C0_spec[1]*H1a7);
	...
	complex float K1=-(C[6]+C[5]/3.)*F_orth + 4./9.*alpha_s_mub/4./pi*(pow(mass_b_mub/m_B,2.)*C0[8]*X_orth-C0[1]*(2./3.*F_orth-G_orth)+r1); 	

	complex float K2u=lambda_u_lambda_c*(C[1]+C[2]/3.)+(C[4]+C[3]/3.)+4./9.*alpha_s_mub/4./pi*(C0[1]*(2./3.-H_orth)+r2); 
	
	complex float K2d=(C[4]+C[3]/3.)+4./9.*alpha_s_mub/4./pi*(C0[1]*(2./3.-H_orth)+r2); 
